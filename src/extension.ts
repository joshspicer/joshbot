...
export function activate(context: vscode.ExtensionContext) {
	...
	const sessionProvider = new class implements vscode.ChatSessionItemProvider, vscode.ChatSessionContentProvider {
		...
		async provideHandleOptionsChange(resource: vscode.Uri, updates: ReadonlyArray<vscode.ChatSessionOptionUpdate>, token: vscode.CancellationToken): Promise<void> {
			...
			let currentOptions = _sessionOptions.get(sessionId) || {};
			
			// Apply updates
			for (const update of updates) {
				if (update.value === undefined) {
					// Remove the option if value is undefined
					const { [update.optionId]: _, ...rest } = currentOptions;
					currentOptions = rest;
				} else {
					currentOptions = { ...currentOptions, [update.optionId]: update.value };
				}
			}
			
			// Store the updated options for this session
			_sessionOptions.set(sessionId, currentOptions);
			
			// Log the changes for debugging
			console.log(`Session ${sessionId} options updated:`, currentOptions);
...
async function handleCreation(accepted: boolean, request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream): Promise<void> {
	...
	const untitledSessionId = original.resource.toString();
